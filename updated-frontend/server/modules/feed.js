...
/**
 * This module handles feeds-related operations.
 * @module feedModule
 */
const feedModule = {
    /**
     ...
     * @param {string} name - The name of the counter (e.g., "taskHogId")
    ...
    createTask: async (dbHelper, session, data) => {
        ...
        try {
            ...
            const hogId = await feedModule.getNextSequence(dbHelper, 'taskHogId'); 
            const task = await dbHelper.create('feed', {
    ...
    getTask: async (dbHelper, taskId) => {
        ...
        try {
            ...
            const task = await dbHelper.findOne('feed', { _id: taskId }, projection);
    ...
    getTasks: async (dbHelper, session) => {
        ...
        try {
            const tasks = await dbHelper.find('feed', { userId: session.userId });
    ...
    deleteTask: async (dbHelper, session, taskId) => {
        ...
        try {
            ...
            const task = await dbHelper.findOne('feed', { _id: taskId, userId: session.userId });
            if (!task) {
                responseData.status = Status.NOT_FOUND;
                responseData.error = 'Task not found';
                return responseData;
            ...
            await dbHelper.deleteOne('feed', { _id: taskId, userId: session.userId });
